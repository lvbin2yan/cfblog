export interface Env {
  DB: D1Database;
  BUCKET: R2Bucket;         // [需配置] 绑定 R2 存储桶
  IMG_DOMAIN: string;      // [需配置] 图床域名，如 https://img.huba.eu.cc
...
export default {
  ...
  async runHermesCore(env: Env, log: (m: string) => Promise<void>, domain: string, isRandom: boolean) {
    ...
    try {
      ...
      // --- [新增图片处理逻辑] ---
      // 逻辑说明：先搜索抓取图片并存入R2，然后通过正则匹配替换文章中的 [IMAGE_PLACEHOLDER]
      try {
        await log("🖼️ 正在搜索并转存实操配图...");
        const storedImages = await this.fetchAndStoreImage(selected.kw, env, log);
        if (storedImages.length > 0) {
          // 核心替换逻辑：将抓取到的 URL 依次填入 AI 预留的坑位中
          article.content = this.injectImagesToContent(article.content, storedImages);
          await log(`✅ 成功将 ${storedImages.length} 张本地化图片插入到教程步骤中。`);
        }
      } catch (imgErr: any) {
        await log(`⚠️ 图片增强失败，保持原样输出: ${imgErr.message}`);
      }
      // --- [新增逻辑结束] ---
  ...
  // --- 新增图片处理函数 (逻辑: 搜索抓取 -> R2转存 -> 正则顺序替换) ---

  async fetchAndStoreImage(query: string, env: Env, log: any): Promise<string[]> {
    const imageUrls: string[] = [];
    try {
      // 1. 调用 SerpApi 图片搜索引擎
      const res = await fetch(`https://serpapi.com/search?q=${encodeURIComponent(query)}&engine=google_images&api_key=${env.SERP_API_KEY}`);
      const data: any = await res.json();
      const rawImages = data.images_results?.slice(0, 3) || [];

      for (let i = 0; i < rawImages.length; i++) {
        try {
          const originUrl = rawImages[i].original;
          const imgFetch = await fetch(originUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } });
          if (!imgFetch.ok) continue;

          const contentType = imgFetch.headers.get("Content-Type") || "image/jpeg";
          const fileName = `blog/img-${Date.now()}-${i}.${contentType.split('/')[1] || 'jpg'}`;

          // 2. 将图片存入 R2 桶
          await env.BUCKET.put(fileName, await imgFetch.arrayBuffer(), {
            httpMetadata: { contentType: contentType }
          });

          // 3. 构造最终访问 URL
          imageUrls.push(`${env.IMG_DOMAIN}/${fileName}`);
          await log(`   🖼️ 图片已就绪: ${fileName}`);
        } catch (e) { continue; }
      }
    } catch (e) { await log("   ⚠️ 无法获取图片资源"); }
    return imageUrls;
  },

  injectImagesToContent(content: string, urls: string[]): string {
    let result = content;
    // 逻辑：AI 生成内容中存在多个 [IMAGE_PLACEHOLDER...]
    // 我们用正则循环寻找这些占位符，并按抓取图片的顺序依次替换为 Markdown 语法
    urls.forEach(url => {
      result = result.replace(/\[IMAGE_PLACEHOLDER[^\]]*\]/, `\n\n![教程实操图](${url})\n\n`);
    });
    // 如果图片不够，清理剩余的占位符；如果图片多了，剩下的不会被插入
    return result.replace(/\[IMAGE_PLACEHOLDER[^\]]*\]/g, "");
  }
};